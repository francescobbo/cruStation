...
impl Debugger {
    ...
    // let value = cpu.load::<u32>(address);                     println!("Read
    // at {:08x}: {:08x}", address, value);                 } else {
    //                     println!("Usage: read-mem [address] (eg: read-mem
    // bfc01234)")                 }
    ...
    //                 if let Ok(address) =
    // u32::from_str_radix(address.trim_start_matches("0x"), 16) {
    // cpu.debugger.breakpoints.push(address);                 } else {
    //                     println!("Usage: breakpoint [address] (eg: breakpoint
    // bfc01234)")                 }
    ...
    //                     }
    //                 } else {
    //                     println!("Usage: delete-breakpoint [index] (eg:
    // delete-breakpoint 0)");                 }
    ...
    // message");                 println!("  c, continue
    // Resumes emulation");                 println!("  s, step
    // Run 1 instruction and break again");                 println!("  n, next
    // Like step, but skipping function calls");                 println!("  r,
    // regs                      Dumps Cpu registers");
    // println!(" rm, read-mem                  Reads a bus address");
    //                 println!("  b, breakpoint [address]      Sets a breakpoint");
    //                 println!(" lb, list-breakpoints          Lists all
    // breakpoints");                 println!(" db, delete-breakpoint [index]
    // Deletes a breakpoints");                 println!("  q, quit
    // Terminates the emulator");                 false
    //             }
    //             _ => {
    //                 println!(
