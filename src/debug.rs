...
pub struct Debugger {
    ...
    pub triggered: Arc<std::sync::atomic::AtomicBool>,
}
...
impl Debugger {
    ...
    fn run_debug_command(line: String, cpu: &mut Cpu) -> bool {
        ...
        match command {
            ...
            "n" | "next" => {
                let pc = effective_pc(cpu);
                let is_call = Disasm::is_function_call(cpu.load::<4>(pc));
                if is_call {
