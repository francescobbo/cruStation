...
pub struct Debugger {
    readline: Editor<(), DefaultHistory>,
    breakpoints: Vec<u32>,
    pub stepping: bool,
    next_breakpoint: u32,
    last_line: String,
...
impl Debugger {
    pub fn new() -> Debugger {
        Debugger {
            readline: Editor::<(), DefaultHistory>::new().unwrap(),
            breakpoints: vec![],
            stepping: false,
        ...
        loop {
            match cpu.debugger.readline.readline("psx> ") {
                Ok(line) => {
                    ...
                    if Debugger::run_debug_command(line, cpu, system) {
                        break;
                    }
                }
    ...
    fn run_debug_command<B: PsxBus>(line: String, cpu: &mut Cpu, system: &mut B) -> bool {
        ...
        match command {
            ...
            "n" | "next" => {
                let pc = effective_pc(cpu);
                let is_call = Disasm::is_function_call(system.read::<4>(pc));
                if is_call {
            ...
            "rm" | "read-mem" => {
                ...
                if let Ok(address) = u32::from_str_radix(address.trim_start_matches("0x"), 16) {
                    let value = system.read::<4>(address);
